...
import { Star, Clock, Users, ArrowRight, Route } from "lucide-react";
    ...
    <section id="tours" className="relative px-6 py-24">
      <div className="mx-auto max-w-[1200px]">
        <div className="mb-16 text-center">
          <span className="bg-gradient-to-r from-orange-400 to-pink-400 bg-clip-text text-[13px] font-semibold uppercase tracking-[3px] text-transparent">
            Tour Nổi Bật
          </span>
        ...
        {hasTours && tours ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {tours.map((tour, index) => {
              ...
              const image = tour.images[0]?.image_url || tour.destination?.image_url;

              return (
                <Link
                  href={`/tours/${tour.id}`}
                  key={tour.id}
                  className="group relative block h-[460px] w-full overflow-hidden rounded-[2rem] bg-zinc-900 transition-all duration-500 hover:shadow-[0_20px_60px_rgba(0,0,0,0.6)]"
                >
                  {image ? (
                    <Image
                      src={image}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 flex flex-col justify-end p-6 transition-transform duration-500 ease-out group-hover:-translate-y-2">
                    ...
                    {tour.description ? (
                      <p className="mb-4 line-clamp-2 text-[13px] text-white/60 transition-all duration-500 group-hover:text-white/80">
                        {tour.description}
                      </p>
                    ) : null}
          ...
          <LandingEmptyState
            ...
            highlights={[
              ...
              "Giá minh bạch trước khi đặt chỗ",
            ]}
          />
        )}
      </div>
